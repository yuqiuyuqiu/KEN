...
from models import Em_Vgg16 as net
...
class CrossEntropyLoss2d(nn.Module):
    ...
    def forward(self, *inputs):
        ...
        pred, pred1, pred2, pred3, target = tuple(inputs)
        ...
        return 1.0*loss + 0.4*loss1 + 0.4*loss2 + 0.4*loss3
...
def val(args, val_loader, model, criterion):
    ...
    for iter, (input, target) in enumerate(val_loader):
        ...
        if not args.onGPU or torch.cuda.device_count() <= 1:
            pred, pred1, pred2, pred3 = tuple(output)
            #pred = output[0]
            loss = criterion(pred, pred1, pred2, pred3, target_var)#
        else:
...
def trainValidateSegmentation(args):
    ...
    # load the model
    model = net.VGG16Em(args.classes)
    ...
    state_dict = torch.load('./pretrainModel/5stages_vgg16_bn-6c64b313.pth')
    new_keys = []
    new_values = []
    for key, value in zip(state_dict.keys(), state_dict.values()):#['state_dict']
        new_keys.append(key)#'module.' +
...
def main(args):
    ...
    print(args.model_name, maxEpoch, maxmIOU)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='KEN_VGG', help='Model name')
